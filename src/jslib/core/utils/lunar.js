...
export default function (SY, SM)
{
	...
	for (i = 0; i < 42; i++)
	{
		...
		{ //日期内
			...
			if (cld[sD].lDay == 1)
			{ //显示农历月
				lObj[i] = (cld[sD].isLeap ? '闰' : '') + cld[sD].lMonth + '月' + (monthDays(cld[sD].lYear, cld[sD].lMonth) == 29 ? '小' : '大');
			}
			else
			{
				lObj[i] = cDay(cld[sD].lDay);
			}	//显示农历日
